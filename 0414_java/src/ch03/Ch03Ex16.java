...
public class Ch03Ex16 {
	...
	//if(){} if의 수행조건이 true인 경우에 수행됨. 
    //else {} if의 수행조건이 false인 경우에 수행됨.
		
		
		int fNum = (int) (Math.random()*10); //0~9까지 랜덤
		int sNum = (int) (Math.random()*10); //0~9까지 랜덤
		
		
		System.out.println("High / Low Game!");
		System.out.println("first number :"+ fNum);
		System.out.println("두 번째 수는 첫번째 수보다 낮을까요? 높을까요?");
		System.out.println("낮으면 1, 높으면 2, 당신의 선택은? : ");
